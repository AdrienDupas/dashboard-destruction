...
const Loading: React.FC = () => {
  ...
  useEffect(() => {
    // Timer minimum 2 secondes
    const timer = setTimeout(() => setFade(true), 2000);
    return () => clearTimeout(timer);
  }, []);

  // Quand l'animation de fade-out est finie, on cache complètement le loader
  ...
  return (
    <div
      style={{
        ...
        zIndex: 3000,
        animation: fade ? 'fadeOut 0.5s forwards' : undefined,
      }}
...
export default function App({
  ...
  if (!geoData || !muniData) return <Loading />;
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      ...
      >
        <p style={{ fontFamily: 'Open Sans, sans-serif', fontWeight: 'bold', marginTop: '0vw', fontSize: 'clamp(14px, 1.5vw, 24px)', color: 'lightgray' }}>
          Destruction in the Gaza stripe
        </p>
        <p style={{ fontFamily: 'Open Sans, sans-serif', fontWeight: 'normal', fontSize: 'clamp(12px, 1.2vw, 18px)', color: 'lightgray' }}>
          Number of buildings destroyed per km²
        </p>
        <img src="/legende.svg" alt="Legend" style={{ display: 'block', width: '90%', height: 'auto' }} />
        
        {/* Slider */}
        <Box sx={{ mt: 0, width: 'clamp(160px, 12vw, 320px)', mb: 2, p: 1, borderRadius: 0, backgroundColor: 'rgba(255,255,255,0.00)' }}>
          <Typography sx={{ color: 'lightgray', fontWeight: 'normal', fontSize: 'clamp(12px, 1vw, 16px)', mb: 1 }}>
            Meters: {cellSize}
          </Typography>
          <Slider
            min={110}
            ...
            sx={{
              color: 'lightgray',
              '& .MuiSlider-valueLabel': { backgroundColor: 'rgba(56, 56, 56, 1)', color: '#959595ff' },
              '& .MuiSlider-thumb': {
                backgroundColor: 'rgba(189, 189, 189, 1)',
                '&:hover, &.Mui-focusVisible, &.Mui-active': { boxShadow: '0 0 0 8px rgba(255, 255, 255, 0.16)' }
              }
            }}
          />
        </Box>

        <p style={{ fontFamily: 'Open Sans, sans-serif', fontWeight: 'normal', fontSize: 'clamp(12px, 1.2vw, 18px)', color: 'lightgray', marginTop: '0vw' }}>
        ...
        <p style={{ fontFamily: 'Open Sans, sans-serif', fontWeight: 'light', fontSize: 'clamp(10px, 0.9vw, 14px)', color: 'lightgray', marginTop: '1vw' }}>
